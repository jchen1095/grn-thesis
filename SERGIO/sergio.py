...
class sergio(object):
    ...
    def build_graph (self, input_file_taregts, input_file_regs, shared_coop_state = 0):
            ...
            if (shared_coop_state <= 0):
                for row in reader:
                    ...
                    for regId, K, C_state in zip(row[2: 2 + nRegs], row[2+nRegs : 2+2*nRegs], row[2+2*nRegs : 2+3*nRegs]):
                        currInteraction.append((int(regId), float(K), float(C_state), 0)) # last zero shows half-response, it is modified in another method
                    ...
                    self.graph_[int(row[0])]['level'] = -1 # will be modified later
                    allTargets.append(int(row[0]))

                    #if self.dyn_:
                    #    for b in range(self.nBins_):
                    #        binDict[b].append(gene(int(row[0]),'T', b))
            else:
        ...
        with open(input_file_regs,'r') as f:
            ...
            for row in reader:
                ...
                #if self.dyn_:
                #    for b in range(self.nBins_):
                #        binDict[b].append(gene(int(row[0]),'MR', b))
    ...
    def calculate_ssConc_(self):
        """
        //TODO Jen: this function looks promising
            ...
            for binID in range(self.nBins_):
                ...
                for g in currGenes:
                    if g[0].Type == 'MR':
                        ...
                        # print(self.bindict[binID])
                        # print(g[0].ID)
                        # print(self.binDict[binID][g[0].ID])
    ...
    # TODO: add population steps to this function instead of using 10 as default, make sure to modify it in populate_with_parentCells_ as well
        ...
        converged = False
        sim_set = self.binDict[binID]
        nc = len(sim_set[0].Conc)

        print(f"\n=== Starting simulation for binID: {binID} ===")
        print(f"Number of initial cells: {nc}\n")
        ...
        while resume:
            ...
            for gID, g in enumerate(sim_set):
                ...
                currU = np.array([self.binDict[binID][gID].Conc[i][-1] for i in range(nc)])
                currS = np.array([self.binDict[binID][gID].Conc_S[i][-1] for i in range(nc)])
                decay_U = np.copy(prod_rate_S)
                decay_S = np.true_divide(self.decayVector_[gID], self.ratioSp_[gID]) * currS

                print(f"prod_rate_U mean: {np.mean(prod_rate_U):.4f}, decay_U mean: {np.mean(decay_U):.4f}")
                print(f"prod_rate_S mean: {np.mean(prod_rate_S):.4f}, decay_S mean: {np.mean(decay_S):.4f}")

                # Noise for U
                if self.noiseType_ == 'sp':
                    dw = np.random.normal(size=nc)
                    amplitude = self.noiseParamsVector_[gID] * np.sqrt(prod_rate_U)
                    noise_U = amplitude * dw
                elif self.noiseType_ == 'spd':
                    dw = np.random.normal(size=nc)
                    amplitude = self.noiseParamsVector_[gID] * (np.sqrt(prod_rate_U) + np.sqrt(decay_U))
                    noise_U = amplitude * dw
                elif self.noiseType_ == 'dpd':
                    dw_p = np.random.normal(size=nc)
                    dw_d = np.random.normal(size=nc)
                    amplitude_p = self.noiseParamsVector_[gID] * np.sqrt(prod_rate_U)
                    amplitude_d = self.noiseParamsVector_[gID] * np.sqrt(decay_U)
                    noise_U = amplitude_p * dw_p + amplitude_d * dw_d

                # Noise for S
                if self.noiseTypeSp_ == 'sp':
                    dw = np.random.normal(size=nc)
                    amplitude = self.noiseParamsVectorSp_[gID] * np.sqrt(prod_rate_S)
                    noise_S = amplitude * dw
                elif self.noiseTypeSp_ == 'spd':
                    dw = np.random.normal(size=nc)
                    amplitude = self.noiseParamsVectorSp_[gID] * (np.sqrt(prod_rate_S) + np.sqrt(decay_S))
                    noise_S = amplitude * dw
                elif self.noiseTypeSp_ == 'dpd':
                    dw_p = np.random.normal(size=nc)
                    dw_d = np.random.normal(size=nc)
                    amplitude_p = self.noiseParamsVectorSp_[gID] * np.sqrt(prod_rate_S)
                    amplitude_d = self.noiseParamsVectorSp_[gID] * np.sqrt(decay_S)
                    noise_S = amplitude_p * dw_p + amplitude_d * dw_d
                ...
                # Check for weird values before continuing
                if np.any(np.isnan(curr_dU)) or np.any(np.isnan(curr_dS)):
                    print(f"NaN detected in gene {gID} updates! Stopping.")
                    return
                if np.any(np.abs(curr_dU) > 1e5) or np.any(np.abs(curr_dS) > 1e5):
                    print(f"Abnormal large values detected in gene {gID} updates. Check noise or rates.")
                    print("curr_dU max:", np.max(curr_dU), "curr_dS max:", np.max(curr_dS))

                for i in range(nc):
                    new_U = max(0, currU[i] + curr_dU[i])
                    new_S = max(0, currS[i] + curr_dS[i])
                    g.Conc[i].append(new_U)
                    g.Conc_S[i].append(new_S)
            ...
            print(f"Step {step}: Converged = {converged}")

            if self.nConvSteps[binID] == 0 and converged:
                self.nConvSteps[binID] = len(self.binDict[binID][0].Conc[0])
                print(f"Convergence reached at step {step}, total steps recorded: {self.nConvSteps[binID]}")

            if converged:
                resume = self.resume_after_convergence(binID)
                print(f"Resume after convergence: {resume}")
    ...
    def simulate_dynamics(self):
        ...
        for bi in self.binOrders_:
            ...
            print(f"=== Finished bin {bi} in {time.time() - start_time:.2f} seconds ===")
